...
async function initMaze(userId, width = 15, height = 15) {
  ...
  const { maze, startPos, goalPos, portalA, portalB } = generateMaze(
    width,
    height,
  );
...
async function movePlayer(userId, x, y) {
  ...
  if (distance > 1) {
    return {
      success: false,
      error: "Lze se pohybovat pouze na sousední políčko",
    };
  }
  ...
  if (cell.type === 2) {
    ...
    const stats =
      updatedUser.success && updatedUser.data ? updatedUser.data : dbUser;
