  ...
  if (isZoomed) {
    ...
    for (let y = startY; y < endY; y++) {
      for (let x = startX; x < endX; x++) {
        ...
        if (isDarkMode && !visibleCells.has(`${x},${y}`)) {
          ...
          ctx.fillRect(
            (x - cameraX) * zoomedCellSize,
            (y - cameraY) * zoomedCellSize,
            zoomedCellSize,
            zoomedCellSize,
          );
          continue;
    ...
    if (
      ...
      if (!isDarkMode || visibleCells.has(`${goalPos.x},${goalPos.y}`)) {
        ...
        ctx.fillRect(
          (goalPos.x - cameraX) * zoomedCellSize,
          (goalPos.y - cameraY) * zoomedCellSize,
          zoomedCellSize,
          zoomedCellSize,
        );
      }
...
function handleCanvasClick(e, canvas) {
  ...
  if (isZoomed) {
    ...
    if (
      x >= 0 &&
      x < mazeState.maze[0].length &&
      y >= 0 &&
      y < mazeState.maze.length
    ) {
      movePlayer(x, y);
    }
  } else {
...
async function movePlayer(x, y) {
  ...
  try {
    ...
    if (data.stats) {
      document.getElementById("statsWins").textContent =
        data.stats.completedMazes || 0;
      document.getElementById("statsDeath").textContent =
        data.stats.deaths || 0;
...
function toggleZoom() {
  isZoomed = !isZoomed;
  document.getElementById("zoomToggle").textContent = isZoomed
    ? "🔍 Zoom zapnutý"
    : "👁️ Zoom vypnutý";
  document.getElementById("zoomLevelControl").style.display = isZoomed
    ? "block"
    : "none";
  // Reset kamery
  cameraX = 0;
